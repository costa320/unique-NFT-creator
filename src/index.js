/* import { getNewCypherKey } from "./cypher"; */
import { generateRandomSplash, downloadImage } from "./drawer.js";
import { getCurrentChanceSession, createNewChanceSession } from "./helpers.js";
import Chance, { integer } from "chance";

export function _init_() {
  console.log("INIT FUNCTION CALLED");

  console.log("NEW SESSION HAS BEEN CREATED =>", createNewChanceSession());

  generateRandomSplash();
}
export function _downloadImage_() {
  downloadImage();
}

export function _getSession_() {
  return getCurrentChanceSession();
}

/* TODO: All random data with chance needs to be done on the backend... and clear data passed on the frontend 
        P.S. multiple hashes need to be created... 1 each new data... so the hash system needs to be revised
*/
export function Test() {
  const key_Example =
    "";
  /*  console.log(divideString("CANEPANECOSTAMAYAALPHA", 5)); */

  let chance1 = new Chance(key_Example);
  let chance2 = new Chance(key_Example);
  console.log(chance1, chance1.random());
  console.log(chance2, chance2.random());
}
